...
type ValidationResult = { totalRows: number; validRows: number; errorRows: number; totalErrors: number; updateRows?: number; newRows?: number; rows: ValidatedRow[] };
type ImportResult = { created: number; updated: number; errors: number; rejected: number; total: number };
...
function IconDl(){return<svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>;}
...
export default function ImportPage(){
  ...
  function reset(){setStep("upload");setFile(null);setValidation(null);setResult(null);setError(null);setShowOnlyErrors(false);setShowOnlyUpdates(false);}
  ...
  function exportErrors(){
    ...
    const rows=errorRows.map(r=>{
      const row={...r.data};
      row["_erros"]=r.errors.map(e=>`${e.field}: ${e.message}`).join(" | ");
      return row;
    });
    ...
    saveAs(new Blob([buf],{type:"application/octet-stream"}),`erros_importacao_${type}_${new Date().toISOString().slice(0,10)}.xlsx`);
  }

  const displayRows=validation?.rows.filter(r=>{
    if(showOnlyErrors)return r.errors.length>0;
    if(showOnlyUpdates)return r.errors.length===0&&r.isUpdate;
    return true;
  })||[];

  const updateCount=validation?.rows.filter(r=>r.errors.length===0&&r.isUpdate).length||0;

  const displayFields=type==="suppliers"
    ?["razao_social","nome_fantasia","cnpj","email","telefone","responsavel_tf","area_responsavel","criticidade"]
    :["numero_contrato","empresa","fornecedor_cnpj","objeto","valor_mensal","data_inicio","data_termino","tipo_cobranca","responsavel","area_responsavel"];
  ...
  const fieldLabels:Record<string,string>={
    ...
    responsavel_tf:"Responsável T&F",area_responsavel:"Área responsável",link_arquivo:"Link arquivo",
    ...
    data_inicio:"Início",data_termino:"Término",responsavel:"Responsável",
  ...
  const supplierCols=[
    {f:"razao_social",r:true},{f:"cnpj",r:true},{f:"nome_fantasia",r:false},{f:"email",r:false},{f:"telefone",r:false},
    {f:"contato_comercial",r:false},{f:"contato_operacional",r:false},{f:"responsavel_tf",r:false},{f:"area_responsavel",r:false},
    {f:"criticidade",r:false},{f:"observacoes",r:false}
  ];
  const contractCols=[
    {f:"numero_contrato",r:true},{f:"empresa",r:true},{f:"fornecedor_cnpj",r:true},{f:"objeto",r:true},
    {f:"valor_mensal",r:false},{f:"valor_total",r:false},{f:"periodicidade_pagamento",r:true},{f:"tipo_cobranca",r:true},
    {f:"detalhe_cobranca",r:false},{f:"reajuste",r:false},{f:"tipo_reajuste",r:false},{f:"mes_reajuste",r:false},
    {f:"data_inicio",r:true},{f:"data_termino",r:true},{f:"aviso_previo_dias",r:false},{f:"renovacao_automatica",r:false},
    {f:"responsavel",r:true},{f:"area_responsavel",r:false},{f:"link_arquivo",r:false},{f:"observacoes",r:false}
  ];
  ...
  return(
    ...
    <Sidebar active="import"/>
      <main style={{flex:1,padding:"28px 32px",overflowX:"auto"}}>
        ...
        <p style={{fontSize:"13px",color:"#6b7280",margin:"0 0 24px"}}>Importe fornecedores e contratos a partir de uma planilha Excel (.xlsx). CNPJs já existentes serão atualizados automaticamente.</p>
          ...
          <div style={{background:"#fff",border:"1px solid #e0e2e7",borderRadius:12,padding:32}}>
            ...
            <div style={{background:"#f8f9fb",borderRadius:8,padding:16,marginBottom:20}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {(type==="suppliers"?supplierCols:contractCols).map(c=>(
              ...
              <p style={{fontSize:"11px",color:"#9ca3af",margin:"8px 0 0"}}>Campos com * são obrigatórios. CNPJs já existentes serão atualizados.</p>
              <a href={`/api/template?type=${type}`} download style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:10,fontSize:"12px",color:"#534AB7",textDecoration:"none",fontWeight:500}}>
                <IconDl/> Baixar template {type==="suppliers"?"de fornecedores":"de contratos"} (.xlsx)
              </a>
            </div>
        ...
        {step==="preview"&&validation&&(
          <div style={{background:"#fff",border:"1px solid #e0e2e7",borderRadius:12,padding:24}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:12,marginBottom:20}}>
              ...
              <Stat label="Novos" value={validation.newRows||0} color="#0F6E56"/>
              <Stat label="Atualizações" value={validation.updateRows||0} color="#185FA5"/>
            ...
            {validation.errorRows>0&&(
              <div style={{background:"#FAEEDA",color:"#854F0B",padding:"10px 16px",borderRadius:8,fontSize:"13px",marginBottom:16}}>
                {validation.errorRows} linha{validation.errorRows>1?"s":""} com erro{validation.errorRows>1?"s":""}. Apenas as linhas válidas serão importadas.
              </div>
            )}
            {(validation.updateRows||0)>0&&(
              <div style={{background:"#E8F0FE",color:"#185FA5",padding:"10px 16px",borderRadius:8,fontSize:"13px",marginBottom:16}}>
                {validation.updateRows} registro{(validation.updateRows||0)>1?"s":""} já existe{(validation.updateRows||0)>1?"m":""} e {(validation.updateRows||0)>1?"serão atualizados":"será atualizado"}.
              </div>
            )}

            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{setShowOnlyErrors(false);setShowOnlyUpdates(false);}} style={{...tabBtn,fontWeight:!showOnlyErrors&&!showOnlyUpdates?600:400,borderBottom:!showOnlyErrors&&!showOnlyUpdates?"2px solid #1a1a2e":"2px solid transparent"}}>Todas ({validation.totalRows})</button>
                {(validation.updateRows||0)>0&&<button onClick={()=>{setShowOnlyErrors(false);setShowOnlyUpdates(true);}} style={{...tabBtn,fontWeight:showOnlyUpdates?600:400,borderBottom:showOnlyUpdates?"2px solid #185FA5":"2px solid transparent",color:showOnlyUpdates?"#185FA5":"#6b7280"}}>Atualizações ({validation.updateRows})</button>}
                <button onClick={()=>{setShowOnlyErrors(true);setShowOnlyUpdates(false);}} style={{...tabBtn,fontWeight:showOnlyErrors?600:400,borderBottom:showOnlyErrors?"2px solid #A32D2D":"2px solid transparent",color:showOnlyErrors?"#A32D2D":"#6b7280"}}>Somente erros ({validation.errorRows})</button>
              </div>
            </div>
            ...
            <div style={{overflowX:"auto",maxHeight:400,overflow:"auto",border:"1px solid #e0e2e7",borderRadius:8}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px",minWidth:800}}>
                ...
                <tbody>{displayRows.map(r=>{
                  ...
                  const isUpd=!hasError&&r.isUpdate;
                  const errorFields=r.errors.map(e=>e.field);
                  return(<tr key={r.rowIndex} style={{background:hasError?"#fff8f8":isUpd?"#F0F4FF":"transparent",borderBottom:"1px solid #f1f3f5"}}>
                    <td style={{...tdc,fontWeight:600,color:hasError?"#A32D2D":isUpd?"#185FA5":"#6b7280"}}>{r.rowIndex}</td>
                    {displayFields.map(f=>{
                    ...
                    <td style={tdc}>{hasError?<span style={{color:"#A32D2D",fontSize:"11px",fontWeight:500}}>{r.errors.length} erro{r.errors.length>1?"s":""}</span>:isUpd?<span style={{color:"#185FA5",fontSize:"11px",fontWeight:500}}>Atualizar</span>:<span style={{color:"#0F6E56",fontSize:"11px",fontWeight:500}}>Novo</span>}</td>
                  </tr>);
                })}</tbody>
              </table>
            </div>
            ...
            <div style={{display:"flex",gap:10,marginTop:20}}>
              ...
              <button onClick={handleImport} disabled={validation.validRows===0} style={{padding:"10px 28px",background:validation.validRows>0?"#1a1a2e":"#9ca3af",color:"#fff",border:"none",borderRadius:8,cursor:validation.validRows>0?"pointer":"not-allowed",fontSize:"13px",fontWeight:500}}>
                Importar {validation.validRows} registro{validation.validRows!==1?"s":""} ({validation.newRows||0} novo{(validation.newRows||0)!==1?"s":""}, {validation.updateRows||0} atualização{(validation.updateRows||0)!==1?"ões":""})
              </button>
            </div>
          </div>
        )}
        ...
        {step==="done"&&result&&(
          <div style={{background:"#fff",border:"1px solid #e0e2e7",borderRadius:12,padding:32,textAlign:"center"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:12,marginBottom:28,maxWidth:600,margin:"0 auto 28px"}}>
              ...
              <Stat label="Atualizados" value={result.updated} color="#185FA5"/>
              <Stat label="Erros" value={result.errors} color="#A32D2D"/>
              <Stat label="Rejeitados" value={result.rejected} color="#6b7280"/>
            </div>

            {result.rejected>0&&(
              <div style={{background:"#FAEEDA",border:"1px solid #F0D68A",borderRadius:10,padding:"16px 20px",marginBottom:24,maxWidth:500,margin:"0 auto 24px",textAlign:"left"}}>
                <p style={{fontSize:"13px",fontWeight:600,color:"#854F0B",margin:"0 0 6px"}}>{result.rejected} registro{result.rejected>1?"s":""} rejeitado{result.rejected>1?"s":""}</p>
                <p style={{fontSize:"12px",color:"#854F0B",margin:"0 0 12px"}}>Deseja exportar os itens com erro em um arquivo Excel para correção e reimportação?</p>
                <button onClick={exportErrors} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 18px",background:"#fff",color:"#854F0B",border:"1px solid #F0D68A",borderRadius:8,cursor:"pointer",fontSize:"13px",fontWeight:500}}>
                  <IconDl/> Exportar erros (.xlsx)
                </button>
              </div>
            )}
